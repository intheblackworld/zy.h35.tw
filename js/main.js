	...
	methods: {
		...
		setPage: function (dot) {
			...
			if ($(`#${dot}`).hasClass('click-active')) {
				$(`#${dot}`).removeClass('click-active')
			}
			$('.dot').removeClass('click-active')
			$(`#${dot}`).addClass('click-active')
			// setTimeout(() => {
	...
	mounted: function () {
		...
		$(window).on("scroll resize", () => {
			...
			if ($(window).scrollTop() > 30 && $(window).scrollTop() < $("#kv2").offset().top - 30) {
				...
				// if ($(window).scrollTop() > scrolv) {//數值變大時 到區塊2
				// 	if ($('#dot-2').hasClass('click-active')) {
				// 		return 
				// 	}
				// 	setTimeout(() => {
				// 		$('#dot-2').get(0).click()
				// 	}, 200);
				// } else if ($(window).scrollTop() < scrolv) {//數值變小時 到區塊1

				// 	$('#dot-2').removeClass('click-active')
				// 	setTimeout(() => {
				// 		$('#dot-1').get(0).click()
				// 	}, 200);
				// }
